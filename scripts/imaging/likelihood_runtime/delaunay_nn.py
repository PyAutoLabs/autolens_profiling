"""
JAX Profiling: DelaunayNN Imaging Likelihood (Full-Pipeline Runtime)
====================================================================

The **Sibson natural-neighbour sibling** of
``likelihood_runtime/imaging/delaunay.py``. Every knob is deliberately
identical — HST, 3.5" mask, radial-bin over-sampling, 1500-vertex Hilbert image
mesh, MGE-60 lens light, Isothermal + ExternalShear mass, split
regularization, border relocator on — and the only difference is the source
mesh class: ``al.mesh.DelaunayNN`` (Sibson natural-neighbour weights over the
whole circumcircle cavity, continuous through Delaunay diagonal flips) in place
of ``al.mesh.Delaunay`` (C0 barycentric weights inside the containing triangle).
So the single-JIT and vmap numbers here differ from the Delaunay cell's by the
interpolation scheme and nothing else.
...
- Mesh vertices are computed in the **image-plane** via a Hilbert image mesh,
  then ray-traced to the source-plane (rectangular computes directly in
  source-plane).
- Uses **InterpolatorDelaunayNN** (Sibson natural-neighbour interpolation)
...
- Delaunay triangulation itself uses scipy on CPU and cannot be JIT-compiled
  (a ``pure_callback`` with ``vmap_method="sequential"``: one host qhull call
  per vmap lane).
...
with timer.section("model_build"):
    ...
    # registered DelaunayNN target — so the runtime cell and the sampler runs
...
_early_dict_path, _ = resolve_output_paths(
    _cli,
    default_dir=_workspace_root / "results" / "runtime" / "imaging" / "delaunay_nn",
    default_basename=f"delaunay_nn_likelihood_summary_{instrument}_v{al.__version__}",
...
if _cli.vmap_probe:
    ...
    _probe_basename = (
        "vmap_probe_delaunay_nn_sparse" if _cli.use_sparse_operator else "vmap_probe_delaunay_nn"
    )
    probe_path = (
        _cli.output_dir or (_workspace_root / "results" / "runtime" / "imaging" / "delaunay_nn")
...
# ``vram.config.VMAP_BATCH`` carries a ("imaging", "delaunay_nn", "hst") row;
# any un-tabled (cell, instrument) simply returns None from ``vmap_batch_for``
# and the vmap phase is skipped rather than crashing, so a fresh
# ``vmap_probe_delaunay_nn.json`` (Phase A of the A100 submit) always wins.
_batch_resolved, _batch_source = resolve_vmap_batch(
    "imaging",
    "delaunay_nn",
    instrument,
    output_dir=_cli.output_dir
    or (_workspace_root / "results" / "runtime" / "imaging" / "delaunay_nn"),
...
if _vmap_skipped:
    ...
    np.testing.assert_allclose(
        ...
        float(full_result),
        rtol=1e-4,
        err_msg="delaunay_nn: JAX vmap likelihood mismatch",
    )
...
dict_path, chart_path = resolve_output_paths(
    _cli,
    default_dir=_workspace_root / "results" / "runtime" / "imaging" / "delaunay_nn",
    default_basename=f"delaunay_nn_likelihood_summary_{instrument}_v{al_version}",
    cell="delaunay_nn",
)
...
# Must equal the breakdown sibling's ``EXPECTED_LOG_EVIDENCE_HST``
# (``likelihood_breakdown/imaging/delaunay_nn.py``) — same dataset, same mask,
# same mesh, same model, so the eager FitImaging figure_of_merit is the same
# number, scheme for scheme. 1500-pixel Hilbert/DelaunayNN, MGE-60 lens,
# adapt_image=lensed_source; one pin per ``--regularization`` scheme, nested
# under the instrument. A missing instrument *or* scheme resolves to None and
# skips the assertion.
#
# hst/constant_split: pinned 2026-09-05 from the breakdown sibling's first
#   eager CPU run on PyAutoLens v2026.8.17.1 (re-checked through this cell's
#   adapt-image wiring on 2026-09-08: reproduced to the last digit).
