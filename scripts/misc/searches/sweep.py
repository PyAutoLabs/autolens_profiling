...
def _script_path(sampler: str, ds_class: str, model: str) -> Path:
    # Dataset-first leaf layout: group-scale profiling lands under cluster/;
    # datacube nests under interferometer/.
    if ds_class == "datacube":
        return (
            _REPO_ROOT
            / "scripts"
            / "interferometer"
    ...
    ds_dir = "cluster" if ds_class == "group" else ds_class
    return _REPO_ROOT / "scripts" / ds_dir / "searches" / sampler / f"{model}.py"
