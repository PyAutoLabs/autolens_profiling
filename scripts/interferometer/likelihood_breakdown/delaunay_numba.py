"""
Numba CPU Profiling: Delaunay Interferometer Likelihood (Per-Step Breakdown)
============================================================================

Decomposes the **recovered numba w-tilde** interferometer likelihood for the
production Delaunay fiducial (Hilbert image-mesh 1500 vertices from the adapt
image, ``al.mesh.Delaunay`` + ``ConstantSplit`` regularization, ``Isothermal`` +
``ExternalShear`` lens mass, no lens light) into its per-evaluation steps.
...
JAX/FFT sibling ``delaunay.py`` in this directory is the comparator, and its
figure of merit is recorded here as ``jax_reference_log_evidence``.

Decomposition method (as ``scripts/imaging/likelihood_breakdown/delaunay_numba.py``):
...
incremental cost. The Hilbert image-mesh placement and the ``W~`` preload +
dirty image are one-off per dataset, so neither is a per-evaluation step and
neither is timed here.
...
Output
------
``results/breakdown/interferometer/delaunay_numba_breakdown_<instrument>_v<version>.{json,png}``
"""
...
hilbert_pixels = 1500  # 1500-tier production fiducial — matches delaunay.py
regularization_coefficient = 1.0

# The reference scatter kernel is O(N_pix^2 * P^2) per curvature build: ~1 s per
# call at sma, ~8 s at alma. Scale the repeat count so a run stays minutes, not
# hours, on the larger instruments.
N_REPEATS = {"sma": 10}.get(instrument, 3)

# ===================================================================
# Setup — identical fiducial to the JAX delaunay.py sibling
# ===================================================================
...
INVERSION_BUILD_LABEL = "Inversion build (trace+Delaunay+mapper)"
H_LABEL = "Regularization matrix H (ConstantSplit)"
...
def _build_inversion(fit):
    """Ray-trace, triangulate, build the mapper, then wrap it in the numba inversion.

    ``fit.inversion`` does the per-evaluation geometry (tracing the data and mesh
    grids to the source plane, the scipy Delaunay triangulation, border relocation
    and the mapper). ``InversionInterferometerNumba`` then reuses that
...
print(f"  delaunay.py path figure_of_merit = {jax_reference_log_evidence}")
del _jax_fit
...
print("\n" + "=" * 78)
print(
    f"NUMBA CPU DELAUNAY INTERFEROMETER PER-STEP BREAKDOWN — {instrument.upper()} — kernel={arm_kernel} — v{al_version}"
)
print("=" * 78)
...
breakdown_summary = {
    ...
    "jax_reference_note": (
        ...
        "InversionInterferometerSparse — the JAX/FFT path the sibling delaunay.py "
        "profiles. The numba log evidence must match it; only the application of "
        "W~ differs between the two, not the algebra."
    ),
}
...
fig.suptitle(
    f"Numba CPU Delaunay Interferometer Likelihood — Per-Step Breakdown — {instrument.upper()} — kernel={arm_kernel}",
    fontsize=12,
    fontweight="bold",
)
ax.set_title(
    f'AutoLens v{al_version}  |  {pixel_scale}"/px  |  {n_image_pixels} image pixels  |  '
    f"{n_visibilities} visibilities  |  {n_mesh_vertices} vertices  |  "
...
# Pinned 2026-09-07 (v2026.8.17.1, 1500-tier Hilbert/Delaunay fiducial). Delaunay
# repeats are bistable at the ~1e-8 relative level (summation-order
