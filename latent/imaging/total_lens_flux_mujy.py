"""
Latent profiling: total_lens_flux_mujy
========================================

Profiles the total_lens_flux_mujy computation in isolation.

This latent sums the image-plane flux of the lens galaxy (galaxies[0]) and
converts it to microjanskies via the AB-magnitude zero-point. It is the
cheapest of the three flux latents because it reads directly from
``fit.galaxy_image_dict`` without re-tracing through the source plane. The
dominant cost is the magzero-normalised AB-mag conversion, which is a small
handful of transcendental operations on a scalar. Under JIT the whole chain
...
LATENT_KEY = "total_lens_flux_mujy"
