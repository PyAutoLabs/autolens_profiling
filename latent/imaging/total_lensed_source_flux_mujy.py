"""
Latent profiling: total_lensed_source_flux_mujy
=================================================

Profiles the total_lensed_source_flux_mujy computation in isolation.

This latent measures the image-plane integrated flux of the source galaxy
after gravitational lensing, reading from ``fit.galaxy_image_dict[tracer.galaxies[-1]]``
and converting to microjanskies via the AB-magnitude zero-point. The lensed
flux is the observable that constrains both the lens mass model and source
luminosity; comparing it against total_source_flux_mujy gives the magnification.
Under JIT the path is a sum over a masked array followed by two scalar
transcendental operations — typically compiles to the same order of cost as
...
LATENT_KEY = "total_lensed_source_flux_mujy"
