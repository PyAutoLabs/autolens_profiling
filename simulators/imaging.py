"""
Simulator Profiling: Imaging
============================

Profiles `autolens_workspace/scripts/imaging/simulator.py` to pinpoint where
time goes during simulation. Times the following phases:

- Grid + over-sampling setup
- Galaxy and tracer construction
- `tracer.image_2d_from` (eager + JIT comparison)
- `simulator.via_tracer_from` (numpy convolution path — expected bottleneck)
- `solver.solve` (eager only; galaxy-scale is fast)
- FITS + JSON output

Run from the workspace root or any path:
...
dataset_name = "simple"
dataset_path = _workspace_root / "dataset" / "imaging" / dataset_name
...
# === PART 3 — via_tracer_from (bottleneck) ===
...
results_dir = _workspace_root / "results" / "simulators"
...
json_path = results_dir / f"imaging_summary_v{al_version}.json"
...
fig.suptitle("Simulator Profiling: Imaging", fontsize=12, fontweight="bold")
ax.set_title(f'AutoLens v{al_version}  |  100×100 @ 0.1"/px', fontsize=9)
ax.margins(x=0.22)
fig.tight_layout()
chart_path = results_dir / f"imaging_summary_v{al_version}.png"
