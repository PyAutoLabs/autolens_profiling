...
## 2026-08-31 REWIND (read this before the phase table)

**Human directive, 2026-08-31: the programme rewinds.** The epic ran largely
without human review, and every mesh / pixelization result it produced goes to an
unphysical solution — the λ⁴ non-PD overflow flood (finite `log_l` up to
**3e+303** accepted by Nautilus as a best point), **59 %** of Phase-8B best
points parked at the **|e| = 1.41421** box corner, and a `cond ~ 1e15–1e19` NaN
population where the reconstruction solve NaNs despite finite log-det terms. On
top of the failures the human lost track of what had actually been learned. The
rewind is therefore both a quarantine of bad evidence and a restart of the review
discipline. Queue of record: **autolens_profiling#200**.

**Quarantine (executed 2026-08-31, identically on RAL
`/mnt/ral/jnightin/autolens_profiling/output/` and the laptop mirror
`/mnt/c/Users/Jammy/Science/inference_programme/output/`, structure preserved):**

- `output/legacy/searches/…` — **83 MGE run-config dirs** (nautilus 16, nss 13,
  nuts 2, smc 3, multi_start_prodigy 4, multi_start_prodigy_autoconv 45).
  **Reusable, pending batch review.**
- `output/legacy_wrong/searches/…` — **71 mesh / pixelization run-config dirs**
  (nautilus 21 across delaunay, delaunay_matern, delaunay_nn, knn, pixelization,
  slam_source_pix, slam_source_pix_nn; nss 3; multi_start_prodigy 47 across
  delaunay_adapt_split, knn), plus RAL-only `slogdet_ab_harvest` and
  `_void_341874_24`. **Unphysical — never cited except as failure-mode
  documentation.**
- Active `output/searches/` now holds only the point_source and cluster trees.
  `hpc/sync pull` scopes to `output/searches` only and passes no `--delete`, so
  it can never reach into `legacy*/`.
...
source and registry code stands** (PositionsLH fix, blackjax 1.6.2, `_targets.py`,
schema v2, `af.SMC`, NUTS plumbing) — nothing is reverted. Only **run evidence**
and **phase/gate state** rewind. Re-execution is human-driven and step-by-step
under the batch-and-review workflow: human-declared `review-at:` slots, packets at
`PyAutoMind/batches/packets/` (spec `batches/packets/TEMPLATE.md`), with the
laptop mirror as the review surface.

**MGE reuse rule (binding for the redo):**

> Before submitting any MGE run during the redo, check `output/legacy/searches/…`
> (RAL **and** the mirror) for an existing result at the same target/config. If
> one is present and has not yet been ruled on in a batch review packet, **do not
> resubmit** — surface it as a packet member (question → witness → health
> evidence → readout → where-to-look) for a human ruling. Accepted → cite it and
...
| NUTS ever initialized from previous NS/MAP results? | **[SUPERSEDED 2026-08-28] NOW YES (in-library)** | Was "NO — `af.BlackJAXNUTS` is single-chain, diagonal-mass, no covariance/start-vector injection". All three negatives are false since PyAutoFit PR#1522: `BlackJAXNUTS(num_chains=...)` vmaps independent chains, `inverse_mass_matrix=None\|"diagonal"\|"dense"\|ndarray\|Result` selects and seeds the metric, and `initializer=result.start_point_from(n_points, jitter)` / `InitializerParamStartPoints.from_result` injects start vectors by prior path **without rewriting priors**. Driven from this repo via `SEARCHES_NUTS_*` (`searches/README.md` "Gradient MCMC (`nuts`)"); first cell `scripts/imaging/searches/nuts/mge.py`, first A100 probe `submit_search_nuts_imaging_mge_a100_hst_fp64_probe` (autolens_profiling#187). The workspace-side warm-start cache (`_warm_start.py`) built for the SMC prototype is now only needed by that prototype. **PROBED 2026-08-29 (RAL 341981, W11):** the machinery works — 4 vmapped chains, start points injected by prior path, the warm arm lands in the right basin (maxL 31,786.03) — but with the only metric a MAP source can supply (**diagonal**) the posterior does not mix (R̂ 3.89, 400/800 divergences, ESS_min 2.0). Warm-vs-cold is unscorable: the cold arm timed out inside warmup. DECISIONS 2026-08-29. |
...
|---|---|---:|---:|---:|---:|---|
| `imaging/mge/hst` | Nautilus (n_live 200) | 31786.782 | 31690.5 | 831 s total (re-baselined 2026-08-24: 707 s sampler / 775 s total, 10.56 ms/eval at `n_batch` 64 — `methods/nautilus.md`) | 63,800 | The truth bar. Truth basin is the dominant mode. |
...
- **Every harvested artifact passes three provenance checks** before it is
  trusted or cited: a fresh version stamp, a wall consistent with the cell's
  recorded cost, and **no resume marker in its log**. The resume marker is
  engine-specific and both forms must be checked: nested searches print
  `Fit Already Completed`, while a MultiStartGradient resume prints
  `Resuming .* previous samples found` instead (RAL 341892 task 0,
  2026-08-27: a 68 s "run" that passed the single-string check and had to be
  marked INVALID by hand).
...
- **First probe (RAL 341981, 2026-08-29 — W11, DECISIONS 2026-08-29).**
  Warm-vs-cold on MGE/hst at 4 chains, 200 warmup + 200 samples, **diagonal**
  metric. Rate measured: **38.53 ms/eval, 3.42 s/draw, 88.8 evals/draw**. The
  warm arm reaches the right basin (maxL 31,786.03) and then fails every Gate
  C sampling threshold by 2–3 orders of magnitude: split-R̂ **3.89** (want
  <1.01), **400 of 800** sampling draws divergent, ESS per gradient eval
  **1/17,672** (want ≳1/30), **0.088 ESS/min** against Nautilus's 727 on the
  same cell. This does **not** refute H6.1: H6.1 names a *dense/low-rank*
  metric and states in terms that diagonal mass will not suffice, and a MAP
  warm source has no posterior covariance to seed a dense one from — so the
  probe could only run the configuration H6.1 predicts should fail. The cold
  arm TIMED OUT inside window adaptation (>2,726 s), so the A/B is unscorable
  and a cold rate does not exist. **Next probe: dense metric seeded from a
  Nautilus posterior, ≥2 h cold budget.**
...
**STATUS 2026-08-29: IN FLIGHT.** `af.SMC` (blackjax adaptive tempered SMC,
MALA/HMC inner kernel) merged into PyAutoFit and the `smc` cell is wired into
the searches framework — `scripts/imaging/searches/smc/mge.py`, `SEARCHES_SMC_*`
(`searches/README.md`), and a 3-arm A100 probe
(`hpc/batch_gpu/submit_search_smc_imaging_mge_a100_hst_fp64_probe`:
`mala_warm`, `hmc_warm`, `mala_cold`, `--time=2:00:00`,
`source: unmeasured  probe-first: yes`). Nothing is measured yet and **GATE D
is not called.** Note the probe is scoped narrower than the phase text below:
it measures a rate, an acceptance/ESS schedule and — from the cold arm — the
**evidence bridge against Nautilus's 31690.42 nats on `imaging/mge/hst`**,
which is the first independent (gradient-side) check this programme has ever
had on a `log_evidence`. The multimodality and mode-dropped-start tests Gate D
is actually written on come after there is a rate. DECISIONS.md 2026-08-29.
...
**SUPERSEDED 2026-08-31 by the programme REWIND — the redo queue restarts at Phase 1 under batch-and-review (see REWIND section + issue #200).**
...
| W6 | Nautilus `n_batch` scan (MGE + Delaunay) — bounds the "JAX Nautilus" ceiling | #163 | A100, ~30 min | **COMPLETE 2026-08-25** (RAL 339842/339843, 8/8 arms): MGE `n_batch` 64 → 1000 gives **1.775× per eval, 1.463× on wall, 1.594× on ESS/min** (10.56 → 5.95 ms/eval; 670 → 458 s), evals +21 %, ESS/eval −10 %; **logZ is NOT flat at the recommended arm** — nb1000 sits −0.10 nat from nb64, ~9σ of the 0.011-nat seed sd, on a single seed per arm. Delaunay saturates by n_batch=64 at 1.26x. Partly a knob, not the full 4x — `methods/nautilus.md` "n_batch SCAN". **TAIL 2026-08-29 (RAL 341987/341988): the MGE optimum is bracketed at last — `n_batch=2000` is the WALL optimum** (sampler wall 444 s, total 519 s; 4000 turns over at 473 / 540 s) while ms/eval keeps falling to 3.03 ms and evals run away to 156,000, so read the wall column. ESS/min is still rising at 4000 (727 → 769) — posterior-per-minute and fit-per-minute disagree. **The logZ drift is now monotone and worse: −0.2155 nat at nb2000 and −0.2386 at nb4000 vs nb64, ~20σ and ~22σ of the 0.011-nat seed sd** — still one seed per arm, so **still nothing adopted**; the next step is nb ∈ {64, 2000} × 5 seeds, not more scan points. nb4000 runs on allocator retries (VRAM edge). **The delaunay tail (nb 512/1000) is UNRUN** — a cuFFT batched plan needed 25.31 GiB and then 100.97 GiB; needs a chunked/smaller-batch redesign, not a resubmit, and delaunay saturated at 64 anyway. Its submit reported `COMPLETED 0:0` after crashing — exit-code guard added to `activate.sh` (`hpc/README.md` "A crashed job FAILS"). DECISIONS 2026-08-29 |
...
| W4 | Phase 1 targets registry, schema v2 (ESS + reject-inclusive evals), `slam_source_pix`, reference baselines — **HARVESTED 2026-08-26 (RAL 340210): 7 of 11 rows certified; 9 of 13 targets certified after RAL 341879 (2026-08-27).** 2 rows lost to missing `_N_LIVE` presets (fixed; landed as knn + delaunay_matern on 341879); 2 lost on `slam_source_pix_nn` — **a target-configuration error in this repo, not a library defect**: `reg.Adapt` was paired with a Delaunay-family mesh, fixed to `reg.AdaptSplit` (`targets/REFS_V1_HARVEST.md` "cause 2", commit 512b805). The 341908 pilot of that fixed target was recorded as "0 Nautilus calls in 6 h"; **corrected 2026-08-29** from `checkpoint.hdf5` — 90,000 calls, 29 bounds, explored=FALSE, maxL 30,701.3, 0.239 s/eval, killed by an overflow flood (finite log_l to 3e+303 from the non-PD lambda^4 regularization matrix), not by resample thrashing. The cell is affordable; the objective was broken. Targets moved to `al.reg.AdaptSplitPower` on a capped (1e-6, 1e4) prior and refs 5/6/7 are queued to re-run — `targets/REFS_V1_HARVEST.md`, DECISIONS.md 2026-08-29 | #161 | CPU + one A100 bake | rides behind W1/W6 |
| W2 | Phase 4 Stage 2 — Nautilus / Prodigy ± positions × 5 seeds on MGE (NSS arm dropped per Gate A) — **HARVESTED 2026-08-26 (RAL 340114/340115, 15/15), PR#174; re-read 2026-08-27.** Nautilus: logZ and mode unchanged to 0.02 nats, but maxL lower with positions in 5/5 seeds (mean −0.126, t = −3.45, p ≈ 0.026) and wall −3…+7 % — not a strict no-op. Prodigy n256: positions at factor 1e8 degrade it 5/5 → **2/5** under Phase 3's coded rule (1/5 only on a stricter, undeclared 0.04-nat band). **Stage 3 diagnostic (RAL 341892) settled it and Gate B pt 2 was CALLED 2026-08-27** — `phase_04_positions/RESULTS.md` "Stage 2"/"Stage 3" | #160 | A100 ~5 GPU-h | W1 (+ W6 for n_batch) |
| W5 | Phase 8B log-coordinate stepping (Scaler → bijector) — **DISPATCHED (340576, 35/39 arms lost), rerun 341845/341860/341874/341875; 7 arms finished, 6 crashed at results-write (PyAutoFit#1535) and recovered offline from `search_internal.dill` 2026-08-27. F5 clean, F4 amended. **COMPLETE 2026-08-29 — FINAL verdict FALSIFIED 3/4 on 39/39 arms (`preliminary: false`), RAL 341978's 15 arms landed.** F1 now fires on BOTH log-det tiers and on balanced 5v5 counts; F3's knife-edge is gone; F4 fires on all five `knn·logit` seeds. F2 still does not fire and still has one scorable seed. 23/39 best points non-physical (59 %) at the \|e\|=1.41421 corner. PyAutoFit#1540 (merged 2026-08-28) pays two of the three owed items — the in-process F5 bit-exactness test and the bijector × `ClipperPriorBoxJoint` composition; a rerun would need `BijectorPerPath` logit-everywhere-except-`ell_comps`, which is a NEW experiment, not a rescoping. DECISIONS 2026-08-29 | #162, #194 | source + A100 | — (parallel smoothing arc) |
| W9 | REMINDER: make slogdet standard in PyAutoArray | #166 | source | W7 |
| W11 | Phase 6 FIRST NUTS probe — `af.BlackJAXNUTS` warm-vs-cold A/B on `imaging/mge/hst`, 4 chains, 200+200, diagonal metric — **RUN 2026-08-29 (RAL 341981). RATE MEASURED: 38.53 ms/eval, 3.42 s/draw (88.8 evals per draw), 35,523 evals in 1368.8 s** → `wall/rates.py` (imaging, mge, hst, a100, fp64, lanes=4, batch=None) and the submit's WALL-BASIS is now `source: rates`. **The warm arm reaches the right basin (maxL 31,786.03) but its posterior is INVALID** — split-R̂ **3.89**, **400/800 divergences**, ESS_min **2.0**, ESS per gradient eval 1/17,672, ESS/min **0.088** against Nautilus's 727 on the same cell. **H6.1 is untested, not refuted**: it names a *dense/low-rank* metric and this arm could only run `diagonal`, because a MAP warm source has no posterior covariance for PyAutoFit to seed one from. **The A/B is unscorable** — the COLD arm TIMED OUT at 45:26 inside warmup, so cold is >2,726 s and unmeasured (submit `--time` raised to 2:00:00, `headroom: 5.0`). Next: dense metric from a **Nautilus** warm start, ≥2 h cold budget — a new task, not started. GATE C not called. DECISIONS 2026-08-29 | #187, #194 | A100, ~35 min used | W3 (PyAutoFit#1522) |
| W10 | Non-physical `ell_comps`: 1,252/6,240 gradient-lane best points sit outside the unit disk (20.1 %; 31.5 % of final points), max \|e\| = 1.41421 = the box corner. The per-component `ell_comps` box admits 21.5 % non-physical volume, `validate_ell_comps` is silent on JAX tracers, and the clipper is faithful to the wrong box — so lanes settle there, and it is both the Phase-4 degradation channel and the Phase 8B crash channel. A joint disk constraint / reparameterisation is a library question — filed as PyAutoMind draft `feature/autogalaxy/ell_comps_joint_disk_constraint.md`, NOT implemented here | Mind draft | source (PyAutoGalaxy/PyAutoFit) | — |
