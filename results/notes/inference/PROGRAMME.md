...
| W2 | Phase 4 Stage 2 — Nautilus / Prodigy ± positions × 5 seeds on MGE (NSS arm dropped per Gate A) — **HARVESTED 2026-08-26 (RAL 340114/340115, 15/15).** Nautilus: positions are a NO-OP (logZ identical to 0.02 nats across 5 seeds). Prodigy n256: positions DEGRADE it 5/5 → 1/5, and not merely as a scoring artifact. Gate B pt 2 NOT callable — the Prodigy half is confounded by a threshold that may simply be mis-set; ~1 GPU-h threshold/factor diagnostic owed first — `phase_04_positions/RESULTS.md` "Stage 2" | #160 | A100 ~5 GPU-h | W1 (+ W6 for n_batch) |
