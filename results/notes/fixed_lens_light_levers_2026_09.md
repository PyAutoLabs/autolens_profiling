...
`feature/fixed-light-numba-levers-l3` (lever 3, stacked on lever 2).
...
**All three levers are measured**, and this note is their whole record. Phase 3 took the
production numba CPU call from **413.301 ms to 230.031 ms — 1.797× (1.80×), 183.270 ms removed** —
and left the fit's numbers where it found them.
...
**Lever 3's result: a further 1.188× on top of lever 2 (273.194 → 230.031 ms), identity on the
A100's jitted rows *and* bit-identity on its eager ones, and a CPU-only verdict.**
`inversion.log_det_curvature_reg_matrix_term` falls **40.849 → 6.318 ms** — 14.95 % of the call to
2.75 % — because the log determinant of `F + λH` is now read off the Cholesky factor
`fnnls_cholesky` already built on its passive set, via the block-determinant identity, instead of
factorising the same (1500, 1500) matrix a second time; `inversion.curvature_reg_matrix`, folded
into the same change as a `cached_property`, falls 4.536 → 2.281 ms with `n_calls` **2 → 1**. The
row that must not move — `solver.fnnls_cholesky`, whose factor this is — did not
(63.928 → 61.133 ms, −4.4 % against a 31–36 % clean spread, with `solver_stats` identical in the two
arms). Cumulatively the three levers take the merge-base call **413.301 → 230.031 ms, 1.797×**,
removing 183.270 ms. Unlike lever 2, lever 3 **is** bit-identical where production reads: the
witness's fast and dense-forced log dets agree at Δ exactly **0.0 nats**, the `figure_of_merit` is
bit-identical, the `reconstruction` is byte-identical, and both A/B arms print the same
`log_likelihood` **18665.014330695605**; the only three `gates` numbers that move are log-det-derived
at one ULP (+3e-12 and −5e-12 nats). The A100 arm's six `log_evidence_terms` are bit-identical in
both systems, its **eager `xp=np`** log evidence is bit-identical too (lever 2's moved −3.4e-06
nats), and its own **dense** potrf of the same `F + λH` costs **1.139 ms** against 40.849 ms on one
CPU core and 5.344 ms on lever 3's route — so lever 3 is CPU-only on the same footing as lever 2,
with the extra structural reason that the JAX path never runs `fnnls` and therefore has no factor to
read.
...
Eight more caveats belong to **lever 3** only:

- **`--n-repeats 64`, not 16 — the one flag that differs from levers 1 and 2, and it was forced.**
  Job 343355 ran this A/B at lever 1/2's 16 repeats. Its control arm passed the ABBA
  instrumentation-overhead gate at **1.0182**; its feature arm measured **224.330 ms** clean and
  **232.172 ms** instrumented and then died on
  `instrumentation_overhead_ratio 1.0366 > 1.03 over 8 counterbalanced blocks`, taking its result
  JSON with it. The ratio climbs monotonically as the call shortens — **1.0147** at 413.301 ms,
  **1.0167** at 302.709, **1.0221** at 267.448, **1.0182** at 268.681, **1.0366** at 224.330 —
  because the instrumentation's cost is a fixed number of milliseconds and the call it is divided by
  is shrinking. At ~224 ms the 1.03 cap sits **inside the 8-block estimator's own standard error**
  (block ratios spanned 0.92–1.17). Job 343356 re-ran the same A/B at 64 repeats — **32**
  counterbalanced blocks instead of 8 — **applied symmetrically to both arms**; the gate threshold
  and the estimator are untouched. Both arms then passed comfortably, and at **0.9928** and
  **0.9942** (i.e. the instrumented calls came out *faster* than the clean ones by 0.6–0.7 %, which
  is what a well-centred estimator on a scattering host looks like). **When comparing this row's
  `instrumentation_overhead_ratio` with levers 1 and 2, compare 32 blocks against 8.**
- **Lever 3's control arm is lever 2, and at 64 repeats it re-measures lever 2's library 2.1 %
  slow.** Lever 3 is stacked on lever 2, so a merge-base control would measure all three levers and
  attribute the sum to lever 3; the A/B measures the **increment**, control PyAutoArray `0b17c292`
  (lever 2's feature revision) against feature `b4322c3e`. That control came out at **273.194 ms**
  against lever 2's recorded **267.448 ms** — **+2.15 %**. The anchor that licenses chaining the
  rows is therefore **not** this control but **job 343355's 16-repeat control row, 268.681 ms, which
  reproduces lever 2's recorded row to +0.46 %** on identical flags (its log-det site 39.520 against
  39.596 ms, −0.19 %; its `curvature_reg_matrix` 4.296 ms over `n_calls` 2 against 4.081; its
  `log_likelihood` **18665.01433069583**, the same digits as lever 2's row). **Quote the 343355
  bridge row whenever you chain lever 3 onto levers 1 and 2** — that is what it is versioned for.
- **Which cumulative figure to quote: `413.301 → 230.031 ms`, the direct ratio 1.797×** (1.80× to
  two figures, which is what the commit message and the PRs carry). The product of the three
  measured rows is 1.379 × 1.132 × 1.188 = **1.854×**, and the gap is the two control
  re-measurements: +1.00 % at lever 2 and +2.15 % here. Because both re-measurements ran *above* the
  recorded rows, **1.797× is a lower bound**: if job 343356's host was 2.1 % slow for the control
  arm it was 2.1 % slow for the feature arm too, and 230.031 ms is inflated by the same factor.
- **Lever 3 is bit-identical where production reads, and 1 ULP where it does not — "bit-identical"
  is a claim lever 3 can make and lever 2 could not.** Both arms of job 343356 printed the **same**
  `log_likelihood` **18665.014330695605** and the **same** `figure_of_merit_sparse_numba`
  **19705.717581305427**, and the witness's W2/W3 are Δ exactly **0.0** nats on the same matrix.
  Exactly three `gates` numbers move, all of them log-det-derived and all at the last bit: the S3
  mapper-block `log_det_curvature_reg_matrix_term` 8326.70017133483 → 8326.700171334833
  (**+3e-12 nats, relative 2.185e-16 — one ULP**), `figure_of_merit_dense`
  19705.71758130552 → 19705.717581305515 (−5e-12 nats, because the P3 gate's *dense*-formalism
  comparison fit is also a numpy fit and so also takes the fast path), and `P3.d_log_evidence_nats`
  −9.094947e-11 → −8.731149e-11. The design's own figure was **≤ 1.8e-12 nats**; the cluster row
  agrees. Do not quote lever 2's 1e-06-nats language for lever 3.
- **`log_likelihood` still is not a cross-leg comparison.** Within job 343356 the two arms agree
  bit-for-bit, which is lever 3's own equivalence observation. Across jobs it moves: job 343355's
  arms printed **18665.01433069583** at 16 repeats and job 343356's printed
  **18665.014330695605** at 64 — a relative 1.2e-14 on the *same* library, because the NNLS memo
  (`seed_source: "memo"`) carries state from call to call and the number of calls changed. Record
  it; never chain it.
- **"Identity on the A100" means the jitted rows — and this time the eager family is bit-identical
  too.** The JAX path never runs `fnnls`, so no factor exists there for a log-det term to read, and
  the guard is numpy-only by construction. All six `log_evidence_terms` of both systems are
  bit-identical across the two A100 arms, as under lever 2. Unlike lever 2, so are the **eager
  `xp=np`** values that *do* take the fast path: `log_evidence_eager` is **29140.295876199474** in
  both arms, and both systems' `log_evidence_figure_of_merit` are unchanged to the last digit, where
  lever 2's moved −3.4466e-06 nats. The only A100 numbers that move are `library_row`'s two log
  likelihoods, at +2.672e-07 (S0) and +3.603e-07 (S3) nats — an order of magnitude *smaller* than
  the ±2e-06 that row scattered between lever 2's arms, i.e. the same known non-reproducibility of
  that row and not an effect of lever 3.
- **This branch's CI is deliberately RED until PyAutoArray's lever-3 PR merges.**
  `scripts/misc/test/test_fixed_light_numba.py::test_curvature_reg_matrix_is_cached` asserts
  `n_calls == 1` **unconditionally** — that is the pin on the cache — and `lint.yml` checks out
  `PyAutoLabs/PyAutoArray` with no `ref:`, i.e. its default branch. One test therefore fails until
  `b4322c3e` is on `main` (measured: 27 passed against `b4322c3e`, 1 failed against `0b17c292`). The
  cell itself reads the flag off the *installed* class (`CURVATURE_REG_MATRIX_IS_CACHED`, matching
  autonerves' `CachedProperty`) rather than a literal, so one checkout of the cell serves both arms
  of the A/B — a literal `True` would have killed the control arm.
- **No pin exists here either.** `pinned_expected` is `null` and `pinned_drift` empty in both CPU
  arms, and the A100 arms' `mapper_block_log_det_status` is **ASSERTED** at rtol 1e-06 with
  `pinned_drift` empty. Every millisecond is RECORDED; the hard verdicts are the four cell gates,
  the witness's W1–W5 and the A100 cell's own S0 log-det pins.
...
### Lever 3 — jobs 343355 / 343356 / 343357

**Three** SLURM jobs, one node, the same day. Job 343355 is the first attempt at the CPU A/B and it
**FAILED** on the harness's own overhead gate after its control arm had completed; job 343356 is the
re-run at 64 repeats and is the row. The A100 job was submitted only after 343356 had finished, so
no two jobs shared the host.

| | CPU A/B, first attempt | CPU A/B + witness | A100 A/B |
|---|---|---|---|
| Job | **343355**, `FAILED 1:0` | **343356**, `COMPLETED 0:0` | **343357**, `COMPLETED 0:0` |
| Elapsed / MaxRSS | 00:01:50 / 1 774 548 K of 32 GB | 00:02:41 / 2 058 568 K of 32 GB | 00:02:02 / 2 572 892 K of 64 GB |
| Node | **euclid-ral-gpu-1** | **euclid-ral-gpu-1** | **euclid-ral-gpu-1** |
| Partition | `gpu` CPUs-only, `--cpus-per-task=4 --mem=32gb`, **no `--gres`** | same | `gpu`, `--gres=gpu:1 --cpus-per-task=4 --mem=64gb` |
| Device | AMD EPYC 7702 64-Core, `os.cpu_count()` 124, kernel 5.14.0-687.39.1.el9_8 | same | NVIDIA A100 80GB PCIe, `cuda:0`, NVIDIA-SMI 610.57.04 / CUDA UMD 13.3 |
| `--n-repeats` | **16** (8 ABBA blocks) | **64** (32 ABBA blocks) | — |
| Job loadavg, entry → exit | `0.00 0.00 0.00` → (killed) | `0.01 0.10 0.08` → `0.93 0.48 0.23` | `0.40 0.40 0.21` → `1.70 0.90 0.42` |
| Cell | `scripts/imaging/likelihood_breakdown/fixed_light_numba.py` | same | `scripts/imaging/likelihood_breakdown/fixed_light.py` (as lever 2) |
| Python / autolens | 3.12.4 / 2026.8.17.1 | 3.12.4 / 2026.8.17.1 | 2026.8.17.1 |
| `AP_ROOT` rev | `3059992a`, branch `feature/fixed-light-numba-levers-l3` | `c6cb80ec`, same branch | `c6cb80ec`, same branch |
| Outcome | control arm JSON written (**the bridge row**); feature arm aborted on the ABBA gate with no JSON | **the CPU row** | **the A100 row** |

Per-arm checkouts and config names (job 343356 and 343357):

| Arm | PyAutoArray checkout | rev | config name |
|---|---|---|---|
| CPU control (= **lever 2**) | `/mnt/ral/jnightin/PyAuto_wt/fixed-light-numba-levers/PyAutoArray_l2` | `0b17c292afc467dff1dbb0c171888daf8bb62731` | `hpc_ral_cpu_fp64_fixed_light_numba_lever3_control_b_warm_t1` |
| CPU feature | `.../PyAutoArray_l3` | `b4322c3e263192eede6776f6741e231c189b2929` | `hpc_ral_cpu_fp64_fixed_light_numba_lever3_b_warm_t1` |
| CPU witness | `.../PyAutoArray_l3` | `b4322c3e` | `hpc_ral_cpu_fp64_lever3` |
| A100 control (= **lever 2**) | `.../PyAutoArray_l2` | `0b17c292` | `hpc_a100_fp64_fixed_light_lever3_control` |
| A100 feature | `.../PyAutoArray_l3` | `b4322c3e` | `hpc_a100_fp64_fixed_light_lever3` |

The merge-base checkout `PyAutoArray_control` (`5e2bc0f4`) and lever 1's `PyAutoArray_feature`
(`bae9296e`) both still exist on the cluster and were deliberately **not** used by any of the three
jobs.

**The in-job proof that the two arms differ by exactly lever 3** is a four-line per-arm probe taken
against the *imported* module before the cell runs, alongside `autoarray.__file__` and the
checkout's `rev-parse HEAD` (`output/output.343356.out`, lines 38–44 and 145–151):

| probe | control (`0b17c292`) | feature (`b4322c3e`) |
|---|---|---|
| `has log_det_from_passive_cholesky_from` | **False** | **True** |
| `fnnls_cholesky` has `factor` | **False** | **True** |
| `curvature_reg_matrix` is `cached_property` | **False** (property) | **True** (`CachedProperty`) |
| `reconstruction_positive_only_from` has `factor` | **False** | **True** |

The A100 job prints the first of those four in each arm (`output/output.343357.out`, lines 55 and
314) and gets `False` / `True` likewise.

The rest of the stack is the shared RAL install, untouched and identical for all five arms — the
same five revisions as levers 1 and 2 (PyAutoNerves `fac8b17b`, PyAutoFit `27d41e7c`, PyAutoArray
shared-install `5e2bc0f4`, PyAutoGalaxy `840ffde0`, PyAutoLens `ccf9295f`) — and the system under
test is the same HST / Hilbert-Delaunay N=1500 / `AdaptSplit(inner=0.1, outer=10.0,
signal_scale=0.1)` / fp64 / `InversionImagingSparseNumba` configuration, with
`image_pixels_masked` **15 361**, `over_sampled_pixels` **62 752**, `n_edge_zeroed` 0 and
subtracted light flux **4945.524141998385** in both CPU arms. The A100 arms run that same dataset
and mesh on the dense JAX path (`inversion_path: "dense"`, `total_params` 1560, `vmap_batch` 16, the
same three XLA flags as levers 1 and 2) with a **fresh compilation and autotune cache per arm**
(`AUTOTUNE_ENTRIES` count 0 at both arms' entry, separate
`output/jax_cache/fixed_light_lever3_{control,feature}_343357` directories).
...
Cell gates, both CPU arms of job 343356 (same cell, same `gate_thresholds`):

| Gate | control | feature | value |
|---|---|---|---|
| `P1_rebaked_operator_equals_original` | **PASS** | **PASS** | 3/3 arrays identical, identical in both arms |
| `S3_mapper_block_equals_S0` | **PASS** | **PASS** | edge-zeroed 0 vs 0 both arms; log-dets rel diff **0.0** (control) and **2.185e-16** (feature — one ULP, below) |
| `P2_dense_equals_sparse_numba_system` | **PASS** | **PASS** | D 1.297e-15, F(mapper) 3.728e-15 (rtol 1e-09), identical in both arms |
| `P3_dense_equals_sparse_numba_evidence` | **PASS** | **PASS** | Δ −9.094947e-11 (control) / −8.731149e-11 (feature) nats, **0** passive-set differences, 1485 passive both sides both arms |
| `instrumentation_overhead_ratio` (≤ 1.03) | 0.9928 **PASS** | 0.9942 **PASS** | **32** ABBA blocks each (8 for levers 1 and 2) |
| `unattributed_fraction` (≤ 0.05) | 0.00439 | 0.00284 | explicit measured remainder |
| `cached_site_call_count_violations` | `{}` | `{}` | the feature arm carries a **hard `n_calls == 1`** on `inversion.curvature_reg_matrix` that the control arm does not — see below |
| `timing_status` / `contention_warning` | measured / false | measured / false | peak 1-min loadavg **0.72** / **0.91** on 124 cores |

**Exactly three `gates` numbers differ between the two arms, and every one of them is one bit of a
log determinant:**

| field | control | feature | Δ |
|---|---|---|---|
| `S3_mapper_block_equals_S0.log_dets.log_det_curvature_reg_matrix_term.s3` | 8326.70017133483 | 8326.700171334833 | **+3e-12 nats, relative 2.185e-16 — one ULP** (the `s0` value is unchanged in both arms, which is why the gate's own `rel_diff` reads 0.0 in the control and 2.185e-16 in the feature) |
| `P3_dense_equals_sparse_numba_evidence.figure_of_merit_dense` | 19705.71758130552 | 19705.717581305515 | −5e-12 nats |
| `P3_dense_equals_sparse_numba_evidence.d_log_evidence_nats` | −9.094947e-11 | −8.731149e-11 | the difference of the two above |

`figure_of_merit_dense` moving while **`figure_of_merit_sparse_numba` stays bit-identical at
19705.717581305427 in both arms** is the expected reading and is worth stating plainly: the P3
gate's *dense*-formalism comparison fit is also a numpy fit with a positive-only solve, so it also
takes lever 3's fast path, and the gate stays PASS because it compares the two formalisms **within**
an arm. Every other `gates` field — P1, P2, the passive sets, the solver stats, the
`log_det_regularization_matrix_term` log-dets (7670.876885100281 in both arms, lever 2's value,
rel diff 0.0) — is **byte-identical between the arms**.

**The production number is bit-identical.** Both arms' `log_likelihood` is
**18665.014330695605**, and both arms' 128-entry `log_likelihood_sequence` has the same shape from
`iid_seed` 263. That is lever 3's headline equivalence statement, and unlike lever 2 it needs no
tolerance beside it.

Solver state, both arms, identical to the last field: `seed_source` `"memo"`,
`warm_start_fallback` `false`, `n_passive` **1484**, `outer_iterations` 2, `inner_iterations` 1,
`warm_start_errors` 5, `memo_cleared_within_block: false`. The rows are genuinely warm and the
solver saw the same problem in both arms — which is what makes the −2.795 ms on
`solver.fnnls_cholesky` readable as noise rather than as an effect, and it is the direct evidence
for the lever's premise that publishing the factor is free.

Warm-up, both arms: 6 calls, `steady: true`, first call 0.3302 s (control) / 0.2903 s (feature)
including numba compile, last 0.2739 / 0.2294 s. `cpu_over_wall` 1.0004 in both, so no pool went
unpinned.

**The cache is asserted by the measurement, not only described.** The cell's site spec declares
`inversion.curvature_reg_matrix` cached via `CURVATURE_REG_MATRIX_IS_CACHED`, read off the installed
class, which turns `cached_site_call_count_violations` into a hard `n_calls == 1` **in the feature
arm only**. It is `{}` there, with `n_calls` 1 against the control arm's 2.

Witness (`fixed_light_numba_levers_l3_witness.py`, **verdict PASS**, feature arm's subshell,
`slurm_job_id` 343356, scipy 1.17.1, autoarray from `PyAutoArray_l3`):

| Gate | status | evidence |
|---|---|---|
| **W1** the fast path fires | **PASS** | `log_det_from_passive_cholesky_from` called **exactly once** during one `fit.figure_of_merit`, returning **8326.700171334833** in **7.04 ms**, with `k_active` **1485** `== len(passive_set)` 1485, **15** active columns, `matrix_shape` **(1500, 1500)** `==` the reduced matrix's shape, and `_nnls_factor_ids` an `arange`-shaped `(1500,)` array. All four guards recorded open: `xp_is_numpy`, `log_det_method == "cholesky"`, `use_positive_only_solver`, `all_linear_obj_have_regularization` |
| **W2** the log determinant | **PASS** | fast **8326.700171334833** vs dense-forced **8326.700171334833**, **Δ exactly +0.000e+00 nats, relative 0.000e+00** against a **1e-10** gate — **bit-identical** — on an `F + λH` of eigenvalues **1.575208 … 5.253690e+06**, **cond 3.335e+06** (`eigvalsh`, 0.19 s), with `h_identical_between_fits` **true**. The dense route is forced through the library's own fallback by monkeypatching the helper to raise `LinAlgError` (patch fired, 2 calls), not by a hand-rolled Cholesky |
| **W3** the likelihood | **PASS** | `figure_of_merit` fast **19705.717581305427** vs dense-forced **19705.717581305427**, **Δ +0.000e+00 nats, relative 0.000e+00** against a **1e-09** gate — **bit-identical**, and the same digits as the cell's own `figure_of_merit_sparse_numba` in both arms |
| **W4** the cache, and the solve | **PASS** | `curvature_reg_matrix` returns the **same object** on two accesses and is a `cached_property`; the **`reconstruction` is byte-identical** across the fast and dense-forced fits, `max |Δ|` **0.000e+00**, 1485 non-zero of 1500. This is the direct check that publishing the factor changed no arithmetic in the solve |
| **W5** the site, in isolation | **PASS** | same `F + λH` and the same published factor, median of 20 at 1 thread: fast **5.344 ms** vs dense **37.713 ms** = **7.06×**, with one `_xp.add(F, H)` build costing **1.936 ms** of it, and the same-matrix Δ **+0.000e+00 nats**. RECORDED, not pinned — the lever's number is the whole-call A/B |

Witness system block: `InversionImagingSparseNumba`, `InterpolatorDelaunay`, `AdaptSplit`,
`n_params` **1500**, `n_funcs` **0**, `edge_zeroed_pixels` **0**, `F + λH` shape (1500, 1500),
subtracted light flux 4945.524141998385, `log_det_method` `"cholesky"` (the default — `"slogdet"` is
untouched by lever 3), `jax_in_sys_modules` **true** (the unfiled PyAutoArray bug, as in levers 1
and 2).

A100 assertions, both arms (`output/output.343357.out`): **"Pin PASSED: S0 log_det_curvature_reg
matches 8360.401763"** and **"Pin PASSED: S0 log_det_regularization matches 7756.614959"** in each
arm, `mapper_block_log_det_status` **ASSERTED** at rtol 1e-06 with both log-dets' `rel_diff`
**0.0**, and `pinned_drift` **empty** in both JSONs. The S3 certified-active-set sweep reproduces in
both arms — budget 1 uncertified at Δ **+1.472423e-02** nats, budgets 2–8 certified at pass 2 at Δ
**+1.164153e-10** nats, smallest certifying budget **2** — `nnls` is 17 iterations (S3) and 22 (S0)
with `converged: true` and
`full_system_pdip_vs_library_reconstruction_max_abs_diff` **3.254729818991109e-12** in both, and
`peak_bytes.after_all` is **1 810 646 016** in both, the same digits as lever 2's arms.
...
`use_jax: false`, `backend: "numba_cpu"`, `InversionImagingSparseNumba`, `n_threads` 1,
`NUMBA_NUM_THREADS` `"1"`, `OMP`/`OPENBLAS`/`MKL`/`VECLIB`/`NUMEXPR` all `"1"`,
`cpu_over_wall` 1.0004 in both arms. Clean (uninstrumented) calls, **n-repeats 64** (the one flag
that differs from levers 1 and 2 — see Scope), warm-up run to a steady state in 6 calls in both
arms.

| arm | PyAutoArray | mean ms | median ms | min | max | clean spread | ABBA | `inversion.log_det_curvature_reg_matrix_term` ms | as % of call |
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|
| control (**lever 2**) | `0b17c292` | **273.194** | 269.121 | 257.129 | 341.164 | 30.8 % | 0.9928 | **40.849** | 14.95 % |
| feature (lever 3) | `b4322c3e` | **230.031** | 225.985 | 214.246 | 297.637 | 36.3 % | 0.9942 | **6.318** | 2.75 % |

| | value |
|---|---:|
| speedup on means | **1.188×** |
| speedup on medians | **1.191×** |
| whole-call saving | **43.163 ms** (means), 43.136 ms (medians) |
| log-det-`F+λH` site saving | **34.531 ms** (40.849 → 6.318, **6.47×** on that site) |
| `curvature_reg_matrix` cache saving | **2.255 ms** (4.536 → 2.281), `n_calls` **2 → 1** |
| the two changes together | **36.786 ms**; the remaining 6.377 ms of the whole-call saving is every other row's delta, i.e. scatter |
| cumulative vs the merge base, direct | 413.301 → 230.031 ms = **1.797×** (1.80× to two figures), **183.270 ms** removed |
| cumulative as the product of the three measured rows | 1.379 × 1.132 × 1.188 = 1.854× |

**The cumulative chain, and the bridge that licenses it:**
...
| lever 1 re-measured as lever 2's control | `bae9296e` | 302.709 (+1.00 %) | — | — | 343353 |
| lever 2 (sparse log det H) | `0b17c292` | **267.448** | 1.132× | 1.545× | 343353 |
| **lever 2 re-measured, 16 repeats — the bridge row** | `0b17c292` | **268.681 (+0.46 %)** | — | — | **343355** |
| lever 2 re-measured as lever 3's control, 64 repeats | `0b17c292` | 273.194 (+2.15 %) | — | — | 343356 |
| **lever 3 (shared Cholesky log det `F + λH`)** | `b4322c3e` | **230.031** | **1.188×** | **1.797×** | **343356** |

The 16-repeat bridge row is what makes lever 3 chainable onto levers 1 and 2: on identical flags it
reproduces lever 2's recorded call to **+0.46 %** and its log-det site to **−0.19 %** (39.520
against 39.596 ms). The 64-repeat control's **+2.15 %** is the host being 2 % slower during job
343356 than during 343353, and it applies to both of that job's arms equally — which is why
**1.797× is a lower bound**, not an upper one. As a consistency check, job 343355's own aborted
feature arm measured **224.330 ms** clean against its control's 268.681, a ratio of **1.198×** —
the same increment the 64-repeat job measured at 1.188×, from a different host state and a different
block count.

**Both arms ran alone on the host.** `contention.load_average_at_start` is `0.49` (control) and
`0.86` (feature) on **124 cores**, rising to peaks of 0.72 and 0.91; the shell echo says the same
(`0.01 0.10 0.08` at job entry, `0.93 0.48 0.23` at exit). The asymmetry is 0.37 of a core on 124
and it works *against* the feature arm. Both arms stamp `timing_status: "measured"` and
`contention_warning: false`.

Both arms ran 64 clean and 64 instrumented calls in **32** counterbalanced ABBA blocks, and the
decomposition closes exactly: 48 instrumented sites (37 reached) plus the explicit
`unattributed (call − sum of exclusive times)` row sum to the clean call to the last printed digit,
**273.194** and **230.031**. `unattributed` is 1.189 ms (0.44 %) and 0.649 ms (0.28 %) unrescaled,
on a 5 % gate. The instrumented calls came out **0.7 % and 0.6 % faster** than the clean ones
(overhead ratios 0.9928 and 0.9942), which is the sign of an estimator now centred on a host whose
rows scatter 31–36 % — at 8 blocks the same estimator put the feature arm at 1.0366 and killed the
job.

### Host 2: `euclid-ral-gpu-1`, A100 80GB PCIe, JAX/XLA

`backend: "gpu"`, `device: "cuda:0"`, fp64, `--mesh delaunay --regularization adapt_split
--vmap-batch 16`, dense inversion path, the same three XLA flags as levers 1 and 2, a fresh JAX
compilation and autotune cache per arm. The cell is **`fixed_light.py`**, the same as lever 2's —
lever 3's site is the `F + λH` log det, and `fixed_light.py` is the cell that has an explicit row
for it.

| `s3/rows` (ms) | control | feature | Δ | lever 2's feature arm | fiducial (gpu-2) |
|---|---:|---:|---:|---:|---:|
| **`Log det Cholesky (F+λH reduced)`** | **1.125** | **1.139** | +1.3 % | 1.134 | 1.171 |
| `Log det Cholesky (H reduced)` | 1.150 | 1.147 | −0.3 % | 1.126 | 1.204 |
| `curvature_reg_matrix build (dense)` | 4.779 | 4.856 | +1.6 % | 4.835 | 4.918 |
| `Cholesky solve (unconstrained)` | 1.469 | 1.478 | +0.6 % | 1.456 | 1.485 |
| `NNLS PDIP (cell-driven, max_iter 50)` | 28.184 | 28.138 | −0.2 % | 28.217 | 28.347 |
| `log_evidence_terms (eager, PDIP solution)` | 60.385 | 54.593 | −9.6 % (**scatter**, see below) | 54.102 | 63.319 |
| `library_row.s3_ms` | 49.313 | 49.175 | −0.3 % | 49.198 | 49.169 |
| `library_row.s0_ms` | 65.277 | 65.026 | −0.4 % | 65.025 | 70.406 |

**Identity, and this time by construction twice over**: the guard is numpy-only, *and* the JAX path
never runs `fnnls` at all, so no factor exists there to read. The jitted rows move by tenths of a
percent and the numbers do not move at all — including the eager ones, which lever 2's did. See
"Lever 3 — the A100 row and the CPU-only verdict".

## Lever 3 — decomposition

Exclusive (self) time per instrumented site, rescaled by `decomposition_rescale_factor`
(1.008476 control / 1.007349 feature — both above 1 this time, because the instrumented calls came
out faster than the clean ones) so the rows sum to the clean call. `n_sites_instrumented` 48 and
`n_sites_reached` 37 in both arms. Top rows by control time:

| site | group | control ms | feature ms | Δ ms | Δ % |
|---|---|---:|---:|---:|---:|
| `sparse_numba.curvature_matrix` | D and F assembly | 89.396 | 87.785 | −1.610 | −1.8 (noise) |
| `solver.fnnls_cholesky` | Positivity solve | 63.928 | 61.133 | −2.795 | −4.4 (noise — **the row that must not move**) |
| `inversion.log_det_curvature_reg_matrix_term` | Log dets + evidence | **40.849** | **6.318** | **−34.531** | **−84.5** |
| `delaunay.triangulation` | Mesh | 18.960 | 18.885 | −0.074 | −0.4 |
| `sparse_numba.psf_weighted_data` | D and F assembly | 8.442 | 8.450 | +0.009 | +0.1 |
| `to_inversion.lp_linear_func_list_galaxy_dict` | Mapper | 8.336 | 7.034 | −1.302 | −15.6 (noise) |
| `inversion.reconstruction` | Positivity solve | 7.393 | 8.297 | **+0.903** | **+12.2 (the `_xp.add` migration)** |
| `inversion.log_det_regularization_matrix_term` | Log dets + evidence | 6.733 | 6.549 | −0.184 | −2.7 (lever 2's site, flat) |
| `inversion.regularization_matrix` | F + λH | 6.393 | 6.097 | −0.296 | −4.6 (lever 1's site, stable) |
| `inversion.curvature_reg_matrix` | F + λH | **4.536** (`n_calls` **2**) | **2.281** (`n_calls` **1**) | **−2.255** | **−49.7 (the cache)** |
| `fit.blurred_image` | Image + blurring | 2.793 | 2.617 | −0.176 | −6.3 |
| `delaunay.mappings_sizes_weights` | Mapper weights | 2.591 | 2.545 | −0.047 | −1.8 |
| `unattributed (call − sum of exclusive times)` | — | 1.199 | 0.654 | −0.545 | −45.5 |

**Two rows moved because lever 3 moved them, and the count matters as much as the milliseconds.**
`inversion.log_det_curvature_reg_matrix_term` falls **40.849 → 6.318 ms** — 14.95 % of the call to
2.75 % — and `inversion.curvature_reg_matrix` falls 4.536 → 2.281 ms **with `n_calls` 2 → 1**. The
call count *is* the cache: the site was reached twice per evaluation (once for the solve's matrix,
once to hand a matrix to the second factorisation) and is now reached once. The feature arm carries
a hard `n_calls == 1` assertion on that site which the control arm cannot carry, so the cache is
asserted by the measurement itself.

**The row that must not move did not.** `solver.fnnls_cholesky` is 63.928 → 61.133 ms, **−4.4 %**,
against a clean spread of 31–36 % and with `solver_stats` identical in the two arms to the last
field. Lever 3's whole premise is that the factor is *free* — published by reference out of a buffer
the solver already held — so a real cost here would have refuted it. It is noise, in the direction
that flatters the feature arm, and it is the third-largest such delta in a table where the lever's
own row moved 34.5 ms.

**`inversion.reconstruction` grew, and that is the `_xp.add` migration, not a regression.** The fast
path evaluates `reconstruction` (itself a `cached_property`) before reading the factor, and
`curvature_reg_matrix` is now built **once**, by whichever site reaches it first. In the control
arm the 1.936 ms `_xp.add(F, H)` build (witness W5) was paid **twice**, once inside the solve's
matrix access and once inside the log-det term; in the feature arm it is paid once, and it lands in
whichever site touches the property first. `inversion.reconstruction` therefore takes on +0.903 ms
of build it did not previously pay, while `inversion.curvature_reg_matrix` gives up 2.255 ms and
`inversion.log_det_curvature_reg_matrix_term` gives up the whole second factorisation. Read the
**groups**, not this row: `F + λH` 10.997 → 8.429 ms and Positivity solve 73.394 → 71.499 ms, so the
two groups the migration shuffles between fall **4.463 ms** together while the evidence group falls
**34.884 ms**.

**Three more rows moved by more than half a millisecond and all three are noise.**
`sparse_numba.curvature_matrix` (−1.610 ms, −1.8 %), `to_inversion.lp_linear_func_list_galaxy_dict`
(−1.302 ms, −15.6 %) and the `unattributed` remainder (−0.545 ms) sit in groups lever 3 cannot
reach: it changed one method's *source* of a factorisation and one property's caching, and touched
no operator assembly and no mapper. On a host whose per-call scatter is 31–36 % these are the
expected size of repeated-measurement differences.

The whole-call saving (43.163 ms) exceeds the two intended savings (34.531 + 2.255 = 36.786 ms) by
**6.377 ms**, which is — by the additive construction of the decomposition — precisely the sum of
every other row's delta. **That 6.377 ms is scatter, not a secondary effect.** The honest reading is
"the log-det-`F+λH` site fell 34.531 ms, the `curvature_reg_matrix` rebuild fell 2.255 ms, and the
call fell 43.163 ms on a host whose rows scatter 31–36 %".

Groups, before and after (percentages of each arm's own clean call):

| group | control ms | control % | feature ms | feature % |
|---|---:|---:|---:|---:|
| D and F assembly | 98.002 | 35.87 | 96.399 | 41.91 |
| Positivity solve | 73.394 | 26.87 | 71.499 | 31.08 |
| **Log determinants + evidence** | **50.600** | **18.52** | **15.716** | **6.83** |
| Mesh (ray trace, placement, triangulation) | 20.394 | 7.46 | 20.306 | 8.83 |
| Mapper + mapping matrix | 11.913 | 4.36 | 10.574 | 4.60 |
| F + λH | 10.997 | 4.03 | 8.429 | 3.66 |
| Mapper weights (Delaunay) | 3.835 | 1.40 | 3.774 | 1.64 |
| Image + blurring | 2.861 | 1.05 | 2.680 | 1.17 |
| unattributed | 1.199 | 0.44 | 0.654 | 0.28 |

**The published group reading has to be re-cut here, and not only rescaled.** Through levers 1 and 2
the "Log determinants + evidence" group was where a factorisation of `F + λH` was paid, and lever
2's note closed by saying that 39.596 of its remaining 48.854 ms was "the single `F + λH` term lever
3 targets". After lever 3 that group is **15.716 ms, 6.83 % of the call, the fourth largest** — and
it **no longer owns a factorisation at all**. The Cholesky of `F + λH` is still computed exactly
once per evaluation, inside `solver.fnnls_cholesky`, where the Positivity solve group has always
paid for it; lever 3 did not remove a factorisation so much as stop paying for a *second* one. So
the group total is no longer the right handle on "what the evidence costs": 6.318 of the group's
15.716 ms is now a Schur complement over 15 columns plus an `_xp.add`, and the factorisation it
reads is a line item of the 61.133 ms solve. **After lever 3 the call's shape is: assembly 42 %,
solve 31 %, mesh 9 %, log-dets and evidence 7 %.**

## Lever 3 — the A100 row and the CPU-only verdict

The A100 job asks the same question lever 2's did, and gets a firmer answer. Lever 3 cannot be
faster on the GPU because it never runs there — but the reason is structural rather than a guard.
`reconstruction_positive_only_from` branches to its own `jax_nnls` implementation when
`xp.__name__` starts with `"jax"`, so `fnnls_cholesky` — the only function that fills the `factor`
dict — is never reached, and **there is no factor in existence** on the JAX side for a log-det term
to read. The dict the numpy branch would have been handed is cleared on entry and left empty. The
`curvature_reg_matrix` cache is likewise inert under `jit`: the traced graph already computes
`F + λH` once as a single HLO value, so caching a Python property changes no HLO. A moved compile
time or a changed `jit_phases` row would have been a finding; none moved beyond the ±5 % that
`compile_s` scatters by run.

The row that answers "is this worth carrying to the JAX path" is the A100's own **dense** Cholesky
of the same (1500, 1500) `F + λH`:

| factorisation of the same `F + λH` | ms |
|---|---:|
| dense potrf, one CPU core (control arm's in-call site, second factorisation) | **40.849** |
| dense potrf, one CPU core (witness W5, same matrix, median of 20) | 37.713 |
| **shared factor + Schur over 15 active columns, one CPU core** (witness W5) | **5.344** |
| the same, as the in-call site (feature arm, including the `_xp.add` it now pays) | 6.318 |
| **dense potrf, A100, jitted** (`s3/rows`, feature arm) | **1.139** |
| dense potrf, A100, jitted (fiducial record, gpu-2) | 1.171 |

**Verdict: a CPU-only lever, and there is nothing left to carry.** The GPU's dense factorisation is
**35.9× faster than the CPU's dense one** and still **4.7× faster than lever 3's shared-factor
route**. At ~1.1 ms a dense Cholesky of a 1500² SPD matrix on an A100 is launch-bound, not
FLOP-bound; and lever 3 is not even an alternative factorisation one could port — it is the re-use
of a factor that the JAX path's solver never produces. Its sibling row `Log det Cholesky (H
reduced)` at 1.147 ms is the scale check: **on the GPU the two log dets still cost the same**,
whereas on the CPU neither of them is now a full factorisation.

**The numbers did not move on the jitted path.** All six `log_evidence_terms` are bit-identical
across the two arms in **both** systems. S3's six, in full: `chi_squared` 19821.951110654016,
`regularization_term` 928.9375270506275, `log_det_curvature_reg` **8360.401762997288**,
`log_det_regularization` 7756.614958909804, `noise_normalization` −79635.26723742482,
`log_evidence` **29140.295897816348**. S0's differ from those only where the joint system differs
from the source-only one (`chi_squared` 19821.951110653972, `regularization_term`
928.9375270502924, `log_evidence` 29140.295897816537, the two log-dets identical) and are likewise
the same digits in both arms. `log_evidence_pdip`, `log_evidence_cholesky`,
`log_evidence_library_edge_zeroed`, `mapper_block_log_dets`, `cond`,
`curvature_reg_build_max_{abs,rel}_diff_vs_library`, the `nnls` block, the certified-active-set
sweep, `edge_zeroed_pixels`, `pinned_expected` and `peak_bytes` are identical too, and both arms'
S0 log-det pins PASSED.

**And unlike lever 2, the eager `xp=np` family did not move either.** `fixed_light.py` builds its S0
fit eagerly with `xp=np` and derives S3 from it, so those values **do** take lever 3's fast path —
the submit header predicted "bit-identical or a few ULP, not lever 2's 1e-10", and the measurement
is the stronger half of that prediction:

| field | control | feature | Δ |
|---|---|---|---|
| `log_evidence_eager` (= `s0.log_evidence_figure_of_merit`) | 29140.295876199474 | 29140.295876199474 | **0.0 — bit-identical** |
| `s3.log_evidence_figure_of_merit` | 29140.295876199274 | 29140.295876199274 | **0.0 — bit-identical** |
| `s0`/`s3` `d_log_evidence_terms_minus_figure_of_merit` | 2.1617e-05 | 2.1617e-05 | 0.0 |
| (lever 2, for contrast) `log_evidence_eager` | 29140.29587964608 | 29140.295876199474 | −3.4466e-06 nats |

That is a sharper statement than it looks. Lever 2's eager value moved because SuperLU and a dense
Cholesky are two different factorisations; lever 3's does not move because it is **the same
factorisation**, read from a different place. It also closes a loop: lever 3's control arm value
29140.295876199474 is byte-identical to **lever 2's feature arm** value, so the eager number is
reproducible run-to-run across jobs and nodes, and its stillness here is evidence rather than
coincidence.

**Two rows to name as scatter rather than as findings.** `s3/rows["log_evidence_terms (eager,
PDIP)"]` 60.385 → 54.593 ms (−9.6 %) and `s0/rows["Log det Cholesky (F+λH reduced)"]`
1.145 → 1.116 ms (−2.5 %) are the two largest relative movers. Both are eager-Python-dispatch or
launch-bound rows around values that are **bit-identical**, and lever 2's own arms put the first at
60.967 → 54.102 ms — the same two-cluster pattern, with the fiducial's 63.319 ms above both. A row
whose value does not change and which reproduces its own spread across two independent jobs is
being timed, not changed. `s0.cond.seconds` 0.514 → 0.225 is a one-shot `eigvalsh` timing and is not
a row at all.

**`library_row`'s log likelihoods are not bit-reproducible on this cell, as lever 2 already
recorded.** Control vs feature: S0 **+2.672e-07**, S3 **+3.603e-07** nats (relative ~1.2e-11).
Lever 2's arms moved **−2.151e-06** and **+1.601e-06** nats on the same code path with opposite
signs — an order of magnitude larger — so this row scatters at ~1e-06 across jobs and lever 3's
~3e-07 is **inside the noise that row already carried**, not an effect. The identity claim above
rests on `log_evidence_terms` and on the eager family, both of which reproduce bit-for-bit.

## Lever 3 — verdict

1. **Lever 3 ships and is worth 1.188× on top of lever 2.** 273.194 → 230.031 ms mean (1.191× on
   medians), a saving of **43.163 ms**, single-threaded, HST Delaunay N=1500, sparse numba operator,
   route b, memo ON, on a host with no other tenant, over 32 counterbalanced ABBA blocks.
   **Cumulatively the production numba CPU call is 413.301 → 230.031 ms, 1.797× (1.80×), with
   183.270 ms removed** — and because lever 3's job re-measured lever 2's library 2.1 % slow, that
   is a lower bound.
2. **What shipped** (PyAutoArray `b4322c3e263192eede6776f6741e231c189b2929`, stacked on lever 2's
   `0b17c292`, branch `feature/fixed-light-numba-levers-l3`), three changes in one commit:
   - `fnnls_cholesky(..., factor: Optional[dict] = None)` fills an out-dict with `U_buffer` (by
     reference — it is local to the call and untouched after the return, so there is no 18 MB copy),
     `k_active`, `passive_set` in the factor's own row/column order, and `matrix_shape`. **32 lines
     added, 0 removed: there is no arithmetic change and the solve is byte-identical whether or not
     the dict is passed** (witness W4). `reconstruction_positive_only_from` clears the dict on entry
     so a memo-seeded attempt that raised cannot leave a discarded solve's factor behind for the
     retry's caller.
   - `inversion_util.log_det_from_passive_cholesky_from` turns that factor into `log det M` by the
     block-determinant identity
     `log det M = log det M_PP + log det(M_AA − M_APᵀ M_PP⁻¹ M_PA)` — the first term
     `2·Σ log diag(U)` is free off the solver's factor, the second is a Schur complement over the
     **15** active columns: one triangular solve with |A| right-hand sides plus one |A|×|A|
     Cholesky. Either Cholesky failing raises `np.linalg.LinAlgError`, deliberately, because it
     means `M` is not positive-definite either and the dense route is the authority on it. **It
     never returns 0.0 for a matrix it could not factorise.**
   - `AbstractInversion.curvature_reg_matrix` becomes a `cached_property`. Its docstring described
     an in-place add removed in `0766edd4` (2025-06-26) and a cache dropped by the blanket
     `e819fa12` sweep (2025-11-05); two stale `curvature_matrix` docstring copies in
     `imaging/sparse.py` and `imaging/mapping.py` were corrected in the same commit (docstring
     only — both were already `cached_property` and out-of-place).
   `log_det_curvature_reg_matrix_term` evaluates `reconstruction`, then takes the fast path only
   under **every** guard: numpy backend, `log_det_method == "cholesky"`,
   `use_positive_only_solver`, all linear objects regularised, a factor present with
   `k_active > 0`, `matrix_shape` match, and the factor's index set covering the whole reduced
   system — which accepts the `arange(n)` identity subset that `solve_ids_to_keep` returns with
   `use_edge_zeroed_pixels` on but no edge pixels to zero, **this system's production
   configuration**. Anything else, and any `LinAlgError` on the fast path, takes the unchanged dense
   route. `"slogdet"` mode, the JAX path and `log_det_regularization_matrix_term` are untouched.
3. **The design measured its alternatives first, and one of them is still open.** Three shapes were
   timed before any of this was written:
   - **Option A — down-date the solver's full factor** by the 15 columns the active set drops.
     **Rejected on measurement**: a down-date costs **~1.7 ms per column** and 15 of them are
     **26 ms**, i.e. **~65 % of the 40 ms potrf it was meant to replace**.
   - **Option B — the Schur complement of the already-factorised passive block.** **1.97 ms** on
     the isolated kernel against the library's 40–46 ms, exact to ≤ 1.8e-12 nats over six instances
     of the production system (0–5 down-dates, `n_active` 15–16). **This is what shipped.**
   - **Option A′ — permute the active columns last and take one potrf**, folding both fancy-index
     copies away. The permuted potrf measured **41 ms**, and the design's reading is that this
     would win a further **~15–20 ms** by removing the second pass entirely — but the permutation
     perturbs the passive factor at **5e-13**, which reaches the active set's knife-edge
     inclusion decisions and would cost the byte-identical solve. **Parked as candidate lever 4**,
     not rejected.
4. **The hazard the change was partly filed against does not exist.** The folded-in PyAutoMind draft
   `curvature_reg_matrix_rebuilt_every_access.md` claimed `curvature_reg_matrix` was rebuilt on
   every access. The call graph says it was reached **twice** per evaluation, not on every access,
   and the history says why: the in-place form went in `0766edd4` and the cache was dropped by a
   blanket `e819fa12` sweep, not by a decision about this property. **Verdict: the hazard as written
   does not exist**; the 4.536 ms over two calls does, and the `cached_property` reclaims 2.255 ms
   of it. The draft is resolved by this commit rather than carried.
5. **This lever *is* bit-identical, and that distinction from lever 2 is the point.** W2: the fast
   log det and the dense-forced log det of the same matrix agree at **Δ exactly 0.0 nats, relative
   0.0**, against a 1e-10 gate three decades tighter than lever 2's — on a matrix of **cond
   3.335e+06** (eigenvalues 1.575 … 5.2537e+06). W3: `figure_of_merit` **bit-identical**,
   19705.717581305427 both routes. W4: the `reconstruction` **byte-identical**, max |Δ| 0.0. In the
   A/B arms the production `log_likelihood` is **18665.014330695605 in both**, and the only three
   `gates` numbers that move are log-det-derived at **one ULP** (+3e-12 and −5e-12 nats). The design
   target was ≤ 1.8e-12 nats; the cluster row is tighter. **Lever 2's "never write bit-identical"
   warning is lever 2's; lever 3 may write it, and must not be quoted with lever 2's tolerances.**
6. **The fast path was witnessed firing on the production system, with its guards recorded open.**
   W1: exactly **one** call during one `figure_of_merit`, `k_active` **1485** `== len(passive_set)`,
   **15** active columns, `matrix_shape` (1500, 1500) matching the reduced matrix, `_nnls_factor_ids`
   the `arange(1500)` identity subset, value 8326.700171334833 in 7.04 ms. A count of 2 would have
   meant the route was re-entered; a `None`-ish fallback would have meant every other check compared
   the dense route with itself. W5 measures the two routes on **one** matrix and the **same**
   published factor: **5.344 vs 37.713 ms, 7.06×**, against the in-call site ratio of 6.47× — the
   two agree to 9 %, which is this phase's only cross-check of the site instrument, and the gap is
   the `_xp.add(F, H)` (1.936 ms) that the in-call site pays and W5's fast leg counts separately.
7. **The profiling side flipped a pin on purpose, and its CI is red until the library merges.**
   `test_curvature_reg_matrix_is_recomputed_more_than_once` became
   `test_curvature_reg_matrix_is_cached`, asserting `n_calls == 1` **unconditionally** — a
   PyAutoArray that stops caching the matrix must fail loudly. `lint.yml` checks out
   `PyAutoLabs/PyAutoArray` with no `ref:`, so that one test fails until `b4322c3e` is on `main`
   (27 passed against `b4322c3e`; 1 failed against `0b17c292`). That is a stated consequence of
   pinning the cache, not a defect to dodge by weakening the assertion. The *cell's* own site spec
   reads the flag off the installed class instead, because one checkout of the cell has to serve
   both arms of the A/B.
8. **On the A100 the jitted path is identity, the eager path is bit-identical, and the lever is
   CPU-only.** All six `log_evidence_terms` bit-identical in both systems; `log_evidence_eager`
   **29140.295876199474** in both arms, where lever 2's moved −3.4466e-06 nats; the A100's dense
   potrf of the same `F + λH` is **1.139 ms** against 40.849 ms on one CPU core and 5.344 ms on
   lever 3's own route, so there is nothing to carry — and structurally nothing to carry it *from*,
   since the JAX solver never builds a factor.
9. **What it costs.** One extra (n, n) array held alive per live inversion for the cached
   `curvature_reg_matrix` (**18 MB at n=1500**), plus the solver's `U_buffer` kept alive by the
   published factor for the rest of the evaluation (**another 18 MB**) — `+18 MB × 2 per live
   inversion`, against a job whose MaxRSS was 2 058 568 K. `curvature_reg_matrix_reduced` was left a
   plain property deliberately (reached once, ~0.01 ms). And one ordering change:
   `log_det_curvature_reg_matrix_term` now evaluates `reconstruction` first — the likelihood path
   already did so, so production order is unchanged, but a diagnostic asking for only that term now
   pays for the solve.
10. **Where the site now sits.** `inversion.log_det_curvature_reg_matrix_term` was **14.95 % of the
    lever 2 call** and the third-largest site in it; it is now **6.318 ms, 2.75 %**, the
    eighth-largest. The log-determinant-and-evidence group falls 50.600 → 15.716 ms and no longer
    contains a factorisation. What is left of the call is the operator assembly and the solve:
    `sparse_numba.curvature_matrix` 87.785 ms and `solver.fnnls_cholesky` 61.133 ms are together
    **148.918 ms, 64.7 % of the 230.031 ms call**.

## Phase 3 — the whole campaign

Three levers, three A/B jobs each with its own witness and its own A100 pair, one merge base
(`5e2bc0f4`) and one cell (phase 2's Leg D: `fixed_light_numba.py --mesh delaunay --dataset hst
--formalism sparse_numba --routes b --threads 1 --instances iid --nnls-warm-start on`).

| lever | site attacked | site before → after (ms) | whole call (ms) | row ratio | cumulative | A100 |
|---|---|---:|---:|---:|---:|---|
| **1** — numba the split-reg assembly | `inversion.regularization_matrix` | 110.599 → 5.924 | 413.301 → 299.709 | **1.379×** | 1.379× | identity to 0.036 % |
| **2** — sparse log det of H | `inversion.log_det_regularization_matrix_term` | 37.480 → 6.508 | 302.709 → 267.448 | **1.132×** | 1.545× | identity on the jitted rows; eager −3.4e-06 nats |
| **3** — shared Cholesky log det of `F + λH`, + cache `curvature_reg_matrix` | `inversion.log_det_curvature_reg_matrix_term` (and `curvature_reg_matrix`, `n_calls` 2 → 1) | 40.849 → 6.318 (and 4.536 → 2.281) | 273.194 → 230.031 | **1.188×** | **1.797×** | identity on the jitted rows **and** bit-identical eager |

**Phase 3 took the production numba CPU likelihood call from 413.301 ms to 230.031 ms — 1.797×,
183.270 ms removed — and left the fit's numbers where it found them** (lever 1 bit-identical, lever
2 at the matrix's own round-off with cond 6.5e12 quoted beside it, lever 3 bit-identical again). The
three non-solver sites phase 2 handed to phase 3 — H's assembly at 111.5 ms and the two log dets at
40.2 + 37.5 ms, 189 ms of a 404.6 ms call — now cost **18.9 ms** together. Every lever is CPU-only,
each on the strength of its own A100 measurement rather than an argument, and the shape of the call
has inverted: what was a call dominated by regularization algebra is now a call dominated by the
operator assembly (41.9 %) and the positivity solve (31.1 %).

## Next

All three levers are measured, and all three sit in open PRs behind the library-first merge gate
(PyAutoArray #553/#554/lever 3, autolens_profiling #269/#271/lever 3). The residue of the
**230.031 ms** lever 3 call,
from its own decomposition:

| site | feature ms | % of the 230.031 ms call | status |
|---|---:|---:|---|
| `sparse_numba.curvature_matrix` | 87.785 | 38.16 | — (phase 2 concluded; not a phase-3 lever) |
| `solver.fnnls_cholesky` | 61.133 | 26.58 | — (phase 2 closed the solver; lever 3 now **re-uses** its factor) |
| `delaunay.triangulation` | 18.885 | 8.21 | — |
| `sparse_numba.psf_weighted_data` | 8.450 | 3.67 | — |
| `inversion.reconstruction` | 8.297 | 3.61 | — (carries the single `_xp.add(F, H)` build now) |
| `to_inversion.lp_linear_func_list_galaxy_dict` | 7.034 | 3.06 | — |
| `inversion.log_det_regularization_matrix_term` | 6.549 | 2.85 | lever 2, done |
| `inversion.log_det_curvature_reg_matrix_term` | 6.318 | 2.75 | lever 3, done |
| `inversion.regularization_matrix` | 6.097 | 2.65 | lever 1, done |
| `inversion.curvature_reg_matrix` | 2.281 | 0.99 | lever 3's cache, done |

**The residue is now two sites.** `sparse_numba.curvature_matrix` (87.785 ms) and
`solver.fnnls_cholesky` (61.133 ms) are **148.918 ms, 64.7 % of the call**, and phase 2 concluded
both of them: the operator assembly is the sparse-numba kernel itself and the solver was closed by
phase 2's own campaign. Any phase 4 has to argue with one of those two or accept ~230 ms as the
floor of this configuration. The candidates below are what phase 3 turned up on the way.

1. **Lever 4 candidate — option A′, permute the active columns last and take one potrf.** Measured
   during lever 3's design at **41 ms** for the permuted factorisation, with the reading that it
   would win a further **~15–20 ms** by folding both fancy-index copies and the second pass away
   entirely. The cost is that the permutation perturbs the passive factor at **5e-13**, which
   reaches the active set's knife-edge inclusion decisions — so the solve would no longer be
   byte-identical, and lever 3's cleanest property would be given up to get it. It needs its own
   witness design (what does "the same active set" mean at 5e-13?) before it is a lever rather than
   a candidate.
2. **Generalise the fast path to edge-zeroed systems.** `zeroed_pixels > 0` currently falls back to
   the dense route, because the coverage guard demands the factor's index set cover the whole
   reduced system and only the `arange(n)` identity subset is accepted. The production
   configuration *is* the identity subset, which is why the lever landed, but a system with real
   edge zeroing pays the old 40 ms. Generalising the index mapping is a contained follow-up with a
   clear test: the same log det, on a system with `n_edge_zeroed > 0`.
3. **`reconstruction_covariance_matrix` factorises `F + λH` a third time.** It is a diagnostic, so
   it is not in this cell's 230 ms, but it is the same matrix and the same factor is now available
   to it. Cheap, and it removes the last duplicate factorisation in the class.
4. **S0 systems with linear funcs need a different identity.** The joint S0 system's log det of the
   mapper block is `log det(M_mm) = log det(M) − log det((M⁻¹)_ff)`, not the passive/active
   partition, so lever 3's helper does not apply to route a. Out of scope throughout phase 3 and
   still out of scope; recorded so nobody expects the fast path to fire there.
5. **The GPU session's HLO census is still owed on #267.** The question "does XLA collapse the
   duplicate `F + λH` adds under `jit`" was explicitly routed away from lever 3 to the
   `hst-gpu-non-solver-residue` HLO census, and that census has not run. Lever 3's A100 rows say the
   *cache* changes no HLO; they do not answer the census's question.
6. **The harness's overhead cap versus the call it is measuring is now a cell question, not a
   lever's.** `max_instrumentation_overhead_ratio = 1.03` was calibrated on a ~400 ms call. The
   ratio's fixed cost does not shrink with the call, so it read 1.0147 at 413 ms and 1.0366 at
   224 ms and killed a job that had measured its row correctly. 64 repeats is the right local
   remedy and it is what this row used, but the general fix belongs to the cell: either scale the
   cap with the call length, or require a minimum block count for the assertion to fire at all
   (`min_blocks_for_overhead_assert` is 3 today), or state the cap as a per-call millisecond budget
   rather than a ratio. **Whoever shortens this call further will hit it again.**
7. **Do not carry either log-det lever to the JAX path.** Both questions are now answered with
   measurements rather than arguments: the A100's dense Cholesky of H is 1.126–1.147 ms against the
   CPU's 37 ms dense / 6.3 ms sparse (lever 2), and its dense Cholesky of `F + λH` is 1.139 ms
   against the CPU's 40.8 ms dense / 5.3 ms shared-factor (lever 3). Both terms are launch-bound
   there, and lever 3's route in particular has no JAX factor to read.
8. **If either compaction constant is ever retuned**, the cell must be `DelaunayNN`. This Delaunay
   row cannot observe them (K = 4 ⇒ `kc` = 4 ⇒ the wide-row branch is never traced), so a Delaunay
   measurement is not evidence about them in either direction.
9. **Never lower `SPARSE_LOG_DET_MIN_PIXELS` without re-reading the cross-backend parity pins.** The
...
10. **Still unfiled, carried from phase 2**: the PyAutoArray bug that
    `abstract_ndarray.__getitem__` imports `jax.numpy`, so a numba-only process cannot stay JAX-free
    after the first `FitImaging` (`jax_in_sys_modules: true` in all three of this phase's witness
    JSONs).
11. **Out of scope throughout, as in phase 2**: route a, route c, rectangular meshes, Euclid, the
    source-pixel sweep, and the sparse-operator/profile-subtracted-image bug.

## Artifacts

### Lever 3 — result artifacts added by this commit (**9 files**)

```
results/breakdown/imaging/fixed_light_numba_delaunay_hpc_ral_cpu_fp64_fixed_light_numba_lever3_control_b_warm_t1.{json,png}  # CPU control = LEVER 2, 0b17c292, job 343356, 64 repeats
results/breakdown/imaging/fixed_light_numba_delaunay_hpc_ral_cpu_fp64_fixed_light_numba_lever3_b_warm_t1.{json,png}          # CPU feature, b4322c3e, job 343356, 64 repeats
results/breakdown/imaging/fixed_light_numba_levers_l3_witness_hpc_ral_cpu_fp64_lever3.json                                   # W1-W5, feature arm, job 343356
results/breakdown/imaging/fixed_light_delaunay_hpc_a100_fp64_fixed_light_lever3_control.{json,png}                            # A100 control = lever 2, 0b17c292, job 343357
results/breakdown/imaging/fixed_light_delaunay_hpc_a100_fp64_fixed_light_lever3.{json,png}                                    # A100 feature, b4322c3e, job 343357
```

Read alongside (already versioned, **not** part of lever 3's A/B):
...
results/breakdown/imaging/fixed_light_numba_delaunay_hpc_ral_cpu_fp64_fixed_light_numba_lever2_b_warm_t1.json        # lever 2's recorded row, 267.448 ms — what the bridge row anchors to
```

**Job 343355's 16-repeat control JSON+PNG — the bridge row — are deliberately *not* versioned**, and
they are the one lever-3 artifact that has to be found rather than read from `results/`. The job
failed on its feature arm's ABBA gate, so it is not an A/B pair and has no place beside the others;
but its control row (**268.681 ms**, ABBA 1.0182 PASS) is what reproduces lever 2's recorded
267.448 ms to +0.46 % and therefore what licenses chaining lever 3 onto levers 1 and 2. They are
kept in this worktree's untracked `output/ral_job343355_16repeats/` and on the cluster at
`/mnt/ral/jnightin/autolens_profiling_wt/fixed-light-numba-levers/output/ral_job343355_16repeats/`:

```
output/ral_job343355_16repeats/fixed_light_numba_delaunay_hpc_ral_cpu_fp64_fixed_light_numba_lever3_control_b_warm_t1.{json,png}
```

Cells and submit scripts added for lever 3:

```
scripts/imaging/likelihood_breakdown/fixed_light_numba_levers_l3_witness.py                # lever 3's W1-W5 witness
scripts/imaging/likelihood_breakdown/fixed_light_numba.py                                  # the CPU cell, all three levers; its curvature_reg_matrix site now declares cached=CURVATURE_REG_MATRIX_IS_CACHED, read off the installed class
scripts/misc/test/test_fixed_light_numba.py                                                # test_curvature_reg_matrix_is_cached, n_calls == 1 unconditionally — the pin, and the reason CI is red until the library merges
scripts/imaging/likelihood_breakdown/fixed_light.py                                        # lever 3's A100 cell (unchanged; the same cell lever 2 used)
scripts/misc/likelihood_breakdown/fixed_light_numpy_solvers.py                              # numpy_solver_injected.patched() accepts and forwards the new `factor` keyword
hpc/batch_cpu/submit_breakdown_imaging_fixed_light_numba_levers_l3_delaunay_ral_hst_fp64    # jobs 343355 (16 repeats, FAILED) and 343356 (64 repeats, the row), CPU arms + witness
hpc/batch_gpu/submit_breakdown_imaging_fixed_light_numba_levers_l3_delaunay_a100_hst_fp64   # job 343357, A100 arms
```
...
| 3 | `b4322c3e263192eede6776f6741e231c189b2929` | `feature/fixed-light-numba-levers-l3` | `inversion/inversion/abstract.py` (+138, the `factor` plumbing, the guarded fast path, `curvature_reg_matrix` → `cached_property`), `inversion_util.py` (+110, the new `log_det_from_passive_cholesky_from`), `util/fnnls.py` (+32/−0, the `factor` out-dict — **no arithmetic change**), `imaging/{mapping,sparse}.py` (+9 each, two stale `curvature_matrix` docstrings), `test_autoarray/inversion/inversion/test_log_det_shared_factor.py` (+569, **35 tests**) and `test_autoarray/util/test_jax_nnls.py` (+7, one test double forwards the new keyword). Totals **7 files, +856/−18**; `pytest test_autoarray/inversion` **605 passed**, `test_autoarray/util` **127 passed** |

Job logs (SLURM, **not versioned** — `output/` is gitignored): `output/output.34334{5,6}.out`
(lever 1), `output/output.34335{3,4}.out` (lever 2) and `output/output.34335{5,6,7}.out` (lever 3),
with the matching `error/error.*.err`, on
`/mnt/ral/jnightin/autolens_profiling_wt/fixed-light-numba-levers` and mirrored into this worktree's
untracked `output/`, alongside `output/ral_job343355_16repeats/` (the bridge row's JSON+PNG).
`output/output.343355.out` is worth keeping open beside lever 3's Scope: it is the only place the
aborted feature arm's measured 224.330 ms and the ABBA gate's own failure message survive, since
that arm never wrote a JSON.

Lever 3's A100 JSONs are `fixed_light.py` records on the same keys as lever 2's, and its witness
JSON carries `results.W1_fast_path_fires` … `results.W5_site_timing_fast_vs_dense` (W2's
`d_log_det_nats` / `relative_difference` / `bit_identical`, W4's `reconstruction_byte_identical` and
`curvature_reg_matrix_same_object`, W5's `fast_ms_median` / `dense_ms_median` /
`add_f_plus_h_ms_median`) plus a `system` block with the `F + λH` eigenvalue extremes and condition
number and the fast-path guard dictionary. Its CPU JSONs add nothing new beyond
`configuration.n_repeats: 64` and `abba.n_blocks: 32`.
...
<!-- Phase 3 is closed: all three levers measured, each with its own A/B job, witness and A100
     pair. A phase 4 starts from "## Next" above, and from the two sites that are 65 % of the
     230.031 ms call: sparse_numba.curvature_matrix and solver.fnnls_cholesky. -->
